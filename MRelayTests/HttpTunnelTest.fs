...
[<TestClass>]
type public HttpTunnelRelayTest() =
     let a = 2
     ...
     member x.SetupEncryptRelay(port: int,forwardPort: int)=
         ...
         let t1 = new Thread( fun () -> ignore(new HttpTunnelRelay(port,Dns.GetHostAddresses("127.0.0.1").[0],forwardPort,true)))
         t1.Start()
     ...
     member x.SetupEncryptDecryptRelays(port: int, forwardPort: int)=
         let key = Text.Encoding.ASCII.GetBytes(key)
         let iv = Text.Encoding.ASCII.GetBytes(iv)
         let t1 = new Thread( fun () -> ignore(new HttpTunnelRelay(port,Dns.GetHostAddresses("127.0.0.1").[0],4500,true)))
         let t2 = new Thread( fun () -> ignore(new HttpTunnelRelay(4500,Dns.GetHostAddresses("127.0.0.1").[0],forwardPort,false)))
         t1.Start()
         t2.Start()
